...
1. 노드 버전을 확인해준다. <b>ver 10 이상이어야 한다</b>

```
$node -v
```

2. 타입스크립트 라이브러리를 설치해준다.

```
$npm i typescript -g
```

3. 타입스크립트의 파일을 js파일로 변환해준다.
...
4. 타입스크립트 환경세팅 속성 알아보기
   > https://www.typescriptlang.org/tools

위 사이트에 들어가서 TSConfig Referenct 라는 페이지에 들어가면 tsconfig.json 파일에 환경 세팅 해줄 속성들이 많이 나오니, 필요한 속성들을 이 공식문서를 보고 찾아서 사용하면 된다.

<br>

5. 타입스크립트 플레이 그라운드 소개
